...
В дополнение к [стандартным функциям](https://learn.microsoft.com/ru-ru/dotnet/visual-basic/language-reference/functions/string-functions), добавлено несколько дополнительных:
 ...
 - <a id="fmt"> `fmt(ШаблонныйТекст)` -  Производит замену в тексте подстановочных сиволов на значения заданные в словаре. Подстановочные символы обрамляются символом `%`. Если нужно вывести символ как есть то его необходимо удвоить `%%`. Например: `fmt("Это просто текст. Значение переменной = %val%. Символ процента = %%")`.
...
<a id="GetFilter"></a> Для создания критерия фильтра используйте функцию `GetFilter`. 
