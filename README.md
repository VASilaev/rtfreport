...
# <a id="Excel"></a> Генерация печатных форм в Excel

*Раздел не окончен*

Настройка шаблона ведется только в фале Excel. 

Диапазон набора данных задается именованным диапазоном с именем набора данных дополненного суффиксом ".record". (Вместо конструкции Scan)

Условные конструкции не доступны

Выводимые поля обрамляются фигурными скобками `{}`. Если нужно в тексте использовать фигурную скобку, то экранируем ее символом `\`. Сам символ `\` если нужно воспринимать как символ, то его удваиваем. Текст внутри фигурных скобок воспринимается как выражение и его значение вычисляется динамически.

Функция Img выводит изображение в текущую ячейку. Может принимать просто путь до файла.

Новая функция `Cell` - позволяет по имени ячейки получить ее содержимое, если передать что угодны вторым параметром то содержимое будет удалено
