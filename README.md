# Создание отчетов по шаблонам Word и Excel из Access - генератор отчетов на VBA

VBA-модуль для создания отчетов Microsoft Word и Microsoft Excel из базы данных Microsoft Access на базе шаблонов. 

Проект подходит для создания печатных форм, актов, накладных, договоров, ведомостей и отчетов Master–Detail в приложениях Microsoft Access.

## Почему это удобно

 - **Минимальное знание VBA.** Для нового отчета не нужно писать сложную логику.
Достаточно импортировать модуль, создать шаблон и вызвать одну процедуру.

 - **Логика в шаблоне.** Все правила формирования отчета (какие поля выводить,
где повторять строки, какие блоки показывать при условиях) задаются
непосредственно в файле шаблона в виде разметки.

 - **Шаблонный код.** [Код для построения отчета](#runit) одинаков для всех отчетов:
вы передаете имя шаблона и параметры, остальное делает генератор.

 - **Сохранение форматирования.** Шаблон Word/Excel сохраняет все стили,
формулы, форматирование ячеек и оформление.

 - **Быстрая разработка новых отчетов.** Чтобы добавить новый отчет, достаточно
создать новый файл-шаблон и зарегистрировать его в системе.
...
flowchart LR
    ...
    image@{ shape: lin-cyl, label: "Изображения" }
    variable["Переменные"]
    template@{ shape: doc, label: "Шаблон Word(rtf)/Excel
...
{f(field1)}...
{f(fieldN)}...
..." }
    KRNReport[["Модуль KRNReport"]]
    Document@{ shape: doc, label: "Готовый документ Word/Excel
...
...
..." }
...
- создание отчетов Word из базы данных Microsoft Access;
- создание отчетов Excel из базы данных Microsoft Access на основе файла-шаблона;
- использование шаблонов Word/RTF и Excel с сохранением форматирования;
- подстановка значений полей и переменных;
- выгрзука данных из таблиц и SQL-запросов Microsoft Access по заданному шаблону;
- циклы Scan/EndScan для табличных и повторяющихся данных;
- создание отчетов Master–Detail с вложенными наборами данных;
- условный вывод блоков через If/Else/EndIf;
- вставка изображений и файлов и из полей Attachment;
- генерация QR-кодов, EAN-13 и CODE128;
...
Итерируемые строки - это одна или несколько строк листа шаблона, которые будут повторяться для каждой строки из набора данных. Строки добавляются путем копирования оригинала, поэтому сохраняется форматирование.
...
### Лист книги как шаблон.

Для того что бы создать отдельный лист на каждую итерацию (по листу на каждую строку набор данных), необходимо на листе в любом месте создать итератор с именем `page.record`.

Имя листа берется из запроса `page` из поля с именем `caption`. Если такого поля нет, то берется оригинальное имя листа и дописывается индекс.

Запросы внутри листа должны использовать данные из запроса `page`, что бы формировать срезы данных. Например 

  - Набор данных `page`:

    ```SQL
    SELECT t.id, t.sName as Caption FROM zMaster as t
    ```

  - Набор данных `row`. Обратите внимание, что значения переменных добавляются через конструкцию [`%$...%`](#fmt):

    ```SQL
    SELECT t.* FROM zDetail as t WHERE t.idMaster = %$page.id%
    ``` 
