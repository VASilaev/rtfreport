...
Private Sub pvReedSolomonComputeRemainder( _
    ...
    For lIdx = lDataPos To lDataPos + lDataSize - 1
        ...
        Next
        If lDegree - 1 >= 0 Then baResult(lDegree - 1) = pvReedSolomonMultiply(baGen(lDegree - 1), bFactor)
    Next
End Sub
...
Public Function pvSetModuleBounded(baQrCode() As Byte, ByVal lX As Long, ByVal lY As Long, ByVal bIsDark As Boolean) As Long
    Dim lQrSize         As Long
    Dim lIndex          As Long
...
Function PointToWMF(baQrCode() As Byte) As String
'Dim baQrCode() As Byte
  Dim lQrSize As Long, lX As Long, lY As Long, lX1 As Long, lY1 As Long, lI As Long
  ...
  'QRCodegenEncode "HelloWorld", baQrCode, QRCodegenEcc_LOW, 1, 40, QRCodegenMask_AUTO, True
  lQrSize = QRCodegenGetSize(baQrCode)
  ...
  For lX = 0 To lQrSize
    For lY = 0 To lQrSize
      If QRCodegenGetModule(baQrCode, lX, lY) Then
        lX1 = lX
        Do While lX1 < lQrSize
          If Not QRCodegenGetModule(baQrCode, lX1 + 1, lY) Then Exit Do
          lX1 = lX1 + 1
        Loop
        
        lY1 = lY
        Do While lY1 < lQrSize
          If Not QRCodegenGetModule(baQrCode, lX, lY1 + 1) Then Exit Do
          lY1 = lY1 + 1
        Loop
  ...
  Next
  Size = Len(PointToWMF) / 2 + 9
  PointToWMF = intToByte(1) & _
     intToByte(9) & _
     intToByte(&H100) & _
     ...
     intToByte(0) & PointToWMF

End Function


Public Function QRCodeExcelPostProcess(ByRef pRange, ByRef pParamList, aArg As Variant)
  Dim baQrCode() As Byte
  If QRCodegenEncode(aArg(1), baQrCode, QRCodegenEcc_LOW, 1, 40, QRCodegenMask_AUTO, True) Then
    ...
    Set Image = pRange.Parent.Pictures.Insert(filename)
    Image.Top = pRange.Top
    Image.Left = pRange.Left
    If aArg(0) > 1 Then
      Image.ShapeRange.width = aArg(2)
    ElseIf pRange.mergecells Then
      Image.ShapeRange.width = pRange.MergeArea.width
    Else
      Image.ShapeRange.width = pRange.width
    End If
    If aArg(0) > 2 Then
      Image.ShapeRange.Height = aArg(3)
    ElseIf pRange.mergecells Then
      Image.ShapeRange.Height = pRange.MergeArea.Height
    Else
      Image.ShapeRange.Height = pRange.Height
    End If
  End If
End Function
...
Public Function qrcode(pParamList, aArg As Variant) As String
  If aArg(1) = vbNullString Or aArg(0) = 0 Then
    qrcode = Empty
  ElseIf pParamList("@SYS_Extension") = "rtf" Then
    Dim baQrCode() As Byte
    If QRCodegenEncode(aArg(1), baQrCode, QRCodegenEcc_LOW, 1, 40, QRCodegenMask_AUTO, True) Then
      baQrCode = StrConv(PointToWMF(baQrCode), vbFromUnicode)
  ...
  ElseIf InSet(pParamList("@SYS_Extension"), "xls", "xlsx", "xlsm") Then
    'Позднее связывание что бы при отсуствии кода Excel не подало при компиляции
    Application.Run "ExcelAddCellFormatter", pParamList, "QRCodeExcelPostProcess", aArg
  Else
